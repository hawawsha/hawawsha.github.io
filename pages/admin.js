...
export default function AdminPage() {
  ...
  async function loadRefunds() {
    try {
      const res = await fetch('/api/refund?action=list');
      const data = await res.json();
      setRefunds(data.records || []);
    } catch (e) {
      showToast('خطأ في تحميل الاسترجاعات');
    }
  }
  ...
  async function handleRefundAction(record, action) {
    setActing(record.id);
    try {
      ...
      const data = await res.json();
      if (data.success) {
        showToast(action === 'approve' ? '✅ تمت الموافقة على الاسترجاع!' : '❌ تم رفض الاسترجاع');
        await loadRefunds();
      } else {
        showToast('خطأ في العملية');
      }
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .tabs{display:flex;gap:10px;padding:14px 16px;border-bottom:1px solid #331a5e;}
        .tab{flex:1;padding:10px;border-radius:12px;border:1px solid #331a5e;background:none;color:#b0b0b0;font-family:'Cairo',sans-serif;font-size:0.85em;font-weight:700;cursor:pointer;}
        ...
        .section-title{padding:8px 16px;font-size:0.85em;font-weight:700;color:#d4af37;}
        ...
        <>
              ...
              {pending.map(r => (
                <div key={r.id} className="card">
                  ...
                  <div className="actions">
                    <button className="btn-approve" onClick={() => handleAction(r, 'approve')} disabled={acting === r.id}>
                      {acting === r.id ? '...' : '✅ قبول'}
                    </button>
                    <button className="btn-reject" onClick={() => handleAction(r, 'reject')} disabled={acting === r.id}>
                      {acting === r.id ? '...' : '❌ رفض'}
                    </button>
                  </div>
                </div>
          ...
          {tab === 'refunds' && (
            <>
              <div className="stats">
                <div className="stat">
                  <div className="stat-num" style={{color:'#eab308'}}>{refundsPending.length}</div>
                  <div className="stat-label">قيد الانتظار</div>
                </div>
                <div className="stat">
                  <div className="stat-num" style={{color:'#22c55e'}}>{refundsApproved.length}</div>
                  <div className="stat-label">مقبول</div>
                </div>
                <div className="stat">
                  <div className="stat-num" style={{color:'#ef4444'}}>{refundsRejected.length}</div>
                  <div className="stat-label">مرفوض</div>
                </div>
              </div>

              <div className="section-title">⏳ قيد الانتظار ({refundsPending.length})</div>
              {refundsPending.length === 0 && <div className="empty">لا توجد طلبات استرجاع جديدة</div>}
              {refundsPending.map(r => (
                <div key={r.id} className="card">
                  <div className="card-header">
                    <div className="avatar" style={{background:'linear-gradient(135deg,#6a0dad,#ef4444)'}}>↩️</div>
                    <div>
                      <div className="username">@{r.fields.buyer_username || r.fields.buyerUsername}</div>
                      <div className="refund-info">📦 {r.fields.product_name || r.fields.productName}</div>
                      <div className="amount">π {r.fields.amount_pi || r.fields.amountPi}</div>
                      <div className="date">📅 {r.fields.created_at || 'غير محدد'}</div>
                    </div>
                  </div>
                  <div className="actions">
                    <button className="btn-approve" onClick={() => handleRefundAction(r, 'approve')} disabled={acting === r.id}>
                      {acting === r.id ? '...' : '✅ موافقة'}
                    </button>
                    <button className="btn-reject" onClick={() => handleRefundAction(r, 'reject')} disabled={acting === r.id}>
                      {acting === r.id ? '...' : '❌ رفض'}
                    </button>
                  </div>
                </div>
              ...
              <div className="section-title" style={{marginTop:16}}>✅ المقبولة ({refundsApproved.length})</div>
              {refundsApproved.length === 0 && <div className="empty">لا يوجد استرجاعات مقبولة</div>}
              {refundsApproved.map(r => (
                <div key={r.id} className="card">
                  <div className="card-header">
                    <div className="avatar" style={{background:'linear-gradient(135deg,#16a34a,#22c55e)'}}>✅</div>
                    <div>
                      <div className="username">@{r.fields.buyer_username || r.fields.buyerUsername}</div>
                      <div className="refund-info">📦 {r.fields.product_name || r.fields.productName}</div>
                      <div className="amount">π {r.fields.amount_pi || r.fields.amountPi}</div>
              ...
              <div className="section-title" style={{marginTop:16}}>❌ المرفوضة ({refundsRejected.length})</div>
              {refundsRejected.length === 0 && <div className="empty">لا يوجد استرجاعات مرفوضة</div>}
              {refundsRejected.map(r => (
                <div key={r.id} className="card">
                  <div className="card-header">
                    <div className="avatar" style={{background:'linear-gradient(135deg,#dc2626,#ef4444)'}}>❌</div>
                    <div>
                      <div className="username">@{r.fields.buyer_username || r.fields.buyerUsername}</div>
                      <div className="refund-info">📦 {r.fields.product_name || r.fields.productName}</div>
                      <div className="amount">π {r.fields.amount_pi || r.fields.amountPi}</div>
